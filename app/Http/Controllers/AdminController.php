...
class AdminController extends Controller
{
    ...
    public function getFinapiUser(Request $request)
    {
        if(!$request->email){
            return response()->json(['success' => false, 'errors' => 'Email is required']);
        }

        try{
            $finapiUser = FinapiUser::where('email', $request->email)->first();

            if($finapiUser){
                return response()->json(['success' => true, 'finapiUser' => $finapiUser]);
            } else {
                return response()->json(['success' => false, 'errors' => 'Finapi user not found']);
            }
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'errors' => $e->getMessage()]);
        }
